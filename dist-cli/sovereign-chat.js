...
import { initializeProvider, streamChatWithOllama, chatWithClaude, getModeLabel, loadConfig, saveConfig } from './ai-provider.js';
...
export class SovereignChatSession {
    ...
    displayWelcomeScreen() {
        ...
        logo.split('\n').forEach((line, i) => {
            let tipLine = '';
            if (i === 1)
                tipLine = chalk.gray(' /help - Show all commands');
            else if (i === 2)
                tipLine = chalk.gray(' /trust - Manage trusted directories');
            else if (i === 3)
                tipLine = chalk.gray(' Launched in ') + chalk.white(projectName);
    ...
    async handleTrustCommand(action) {
        if (!action) {
