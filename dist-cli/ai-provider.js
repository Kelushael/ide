 ...
 */
export async function detectOllamaModels(host = 'http://localhost:11434') {
    try {
 ...
 */
export async function getBestOllamaModel(host = 'http://localhost:11434') {
    const available = await detectOllamaModels(host);
    if (available.length === 0)
        return null;
    // Try preferred models first
    for (const preferred of PREFERRED_MODELS) {
        const found = available.find(m => m.toLowerCase().includes(preferred.split(':')[0].toLowerCase()));
        if (found)
            return found;
    }
 ...
 * Chat with Ollama
 */
export async function chatWithOllama(provider, messages) {
    if (!provider.ollama)
 ...
 */
export async function* streamChatWithOllama(provider, messages) {
    if (!provider.ollama)
        throw new Error('Ollama not initialized');
 ...
 * Chat with Claude
 */
export async function chatWithClaude(provider, messages, tools) {
    if (!provider.claude)
 ...
 */
export function getModeLabel(provider) {
