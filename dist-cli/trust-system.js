 ...
 */
async function loadTrustedDirs() {
    try {
 ...
 */
export async function listTrustedDirs() {
    const trusted = await loadTrustedDirs();
    return [...trusted];
}
/**
 ...
 */
export async function untrustCurrentDir() {
    const cwd = process.cwd();
    const trusted = await loadTrustedDirs();
    if (trusted.has(cwd)) {
        trusted.delete(cwd);
 ...
 */
export async function clearAllTrust() {
